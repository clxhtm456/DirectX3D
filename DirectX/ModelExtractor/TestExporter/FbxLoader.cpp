...
void Utility::FbxLoader::DisplayControlsPoints(FbxMesh* pMesh, std::vector<Vertex>& p_vertices, FbxAMatrix* l_transformMatrix)
{
    ...
    for (i = 0; i < lControlPointsCount; i++)
    {
        ...
        l_vertex.position.y = static_cast<float>(lControlPoints[i].Buffer()[1]);
        l_vertex.position.z = static_cast<float>(lControlPoints[i].Buffer()[2]);
        l_vertex.position.w = 1.0f;
...
void Utility::FbxLoader::DisplayPolygons(FbxMesh* pMesh, std::vector<uint32_t>& p_indices, std::vector<Vertex>& p_vertices,FbxAMatrix* l_transformMatrix)
{
            ...
            for (l = 0; l < pMesh->GetElementUVCount(); ++l)
            {
                ...
                case FbxGeometryElement::eByControlPoint:
                    ...
                    case FbxGeometryElement::eDirect:
						...
						p_vertices[lControlPointIndex].uv.x = float(l_uv.mData[0]);
						p_vertices[lControlPointIndex].uv.y = float(l_uv.mData[1]);
            ...
            for (l = 0; l < pMesh->GetElementNormalCount(); ++l)
            {
                ...
                if (leNormal->GetMappingMode() == FbxGeometryElement::eByPolygonVertex)
                {
                    ...
                    case FbxGeometryElement::eDirect:
						...
						p_vertices[lControlPointIndex].normal.x = float(l_normal.mData[0]);
						p_vertices[lControlPointIndex].normal.y = float(l_normal.mData[1]);
						p_vertices[lControlPointIndex].normal.z = float(l_normal.mData[2]);
                ...
                else
                {
                    ...
                    for (int lVertexIndex = 0; lVertexIndex < pMesh->GetControlPointsCount(); lVertexIndex++)
                    {
                        ...
                        p_vertices[lVertexIndex].normal.x = float(lNormal.mData[0]);
                        p_vertices[lVertexIndex].normal.y = float(lNormal.mData[1]);
                        p_vertices[lVertexIndex].normal.z = float(lNormal.mData[2]);
...
bool Utility::FbxLoader::DisplayLink(FbxMesh* pGeometry, FbxTime p_time, FbxAMatrix* p_globalPos,  std::vector<Vertex>& p_vertices, SkeletonAnim& p_anim)
{
    //Display cluster now
    ...
    for (int frameIndex = 0; frameIndex < p_anim.bones.size(); ++frameIndex)
    {
        ...
        for (i = 0; i != lSkinCount; ++i)
        {
            ...
            assert(lClusterCount <= SkeletonAnim::MAX_BONE);
            for (j = 0; j != lClusterCount; ++j)
            {
                ...
                if (lSkinningType == FbxSkin::eLinear || lSkinningType == FbxSkin::eRigid)
                {
					...
					XMMATRIX test;
					p_anim.bones[frameIndex][boneIndex].r[0] = XMVectorSet(float(boneMatrix.Get(0, 0)), float(boneMatrix.Get(0, 1)), float(boneMatrix.Get(0, 2)), float(boneMatrix.Get(0, 3)));
					p_anim.bones[frameIndex][boneIndex].r[1] = XMVectorSet(float(boneMatrix.Get(1, 0)), float(boneMatrix.Get(1, 1)), float(boneMatrix.Get(1, 2)), float(boneMatrix.Get(1, 3)));
					p_anim.bones[frameIndex][boneIndex].r[2] = XMVectorSet(float(boneMatrix.Get(2, 0)), float(boneMatrix.Get(2, 1)), float(boneMatrix.Get(2, 2)), float(boneMatrix.Get(2, 3)));
					p_anim.bones[frameIndex][boneIndex].r[3] = XMVectorSet(float(boneMatrix.Get(3, 0)), float(boneMatrix.Get(3, 1)), float(boneMatrix.Get(3, 2)), float(boneMatrix.Get(3, 3)));
