...
void BinaryWriter::Vector2(const XMFLOAT2& data)
{
	WriteFile(fileHandle, &data, sizeof(XMFLOAT2), &size, NULL);
}

void BinaryWriter::Vector3(const XMFLOAT3& data)
{
	WriteFile(fileHandle, &data, sizeof(XMFLOAT3), &size, NULL);
}

void BinaryWriter::Vector4(const XMFLOAT4& data)
{
...
XMCOLOR BinaryReader::Color3f()
{
	float r = Float();
	float g = Float();
	float b = Float();

	return XMCOLOR(r, g, b, 1.0f);
}

XMCOLOR BinaryReader::Color4f()
{
	float r = Float();
	float g = Float();
	float b = Float();
