...
TerrainEdit * TerrainEdit::Create(UINT horizontal, UINT vertical, UINT textureDetail)
{
	auto pRet = new TerrainEdit();
...
bool TerrainEdit::Init(UINT horizontal, UINT vertical, UINT textureDetail)
{
	shader = Shader::Add(L"TerrainEdit");
...
void TerrainEdit::Render(Camera* viewer)
{
	Super::Render(viewer);
...
void TerrainEdit::BaseMap(wstring file)
{
	baseMap = Texture::Add(file);
}

void TerrainEdit::LayerMap(wstring file, wstring alpha)
{
	alphaMap = Texture::Add(file);
	layerMap = Texture::Add(file);
}

float TerrainEdit::GetHeight(Vector3& position)
{
...
float TerrainEdit::GetHeightPick(Vector3 & position)
{
	UINT x = (UINT)position.x;
	UINT z = (UINT)position.z;
...
Vector3 TerrainEdit::GetPickedPosition()
{
...
void TerrainEdit::SetHeight(float x, float z, float height)
{
}

void TerrainEdit::CreateVertexData()
{
...
void TerrainEdit::CreateIndexData()
{
...
void TerrainEdit::CreateNormalData()
{
...
void TerrainEdit::RaiseHeight(Vector3 & position, UINT type, UINT range)
{
	D3D11_BOX rect;
